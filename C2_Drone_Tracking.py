...
def run_C2(stopTime, saveSimulationFilePath=None, saveFiguresFolderPath=None):
    ...
    # Build the drone aircraft object
    ...
    C2_drone1_tracker = track.noisy_a2a(
        C2["lat"],
        C2["lon"],
        C2["alt"],
        C2["roll"],
    ...
    C2_drone1_tracker.track_target(
        drone1_gnc.lat[-1],
        drone1_gnc.lon[-1],
        drone1_gnc.h[-1],
        drone1_gnc.time[-1])
        ...
        if runSim:
            while drone1_gnc.time[-1] < stopTime:
                ...
                if drone1_gnc.time[-1] >= 1 and drone1_gnc.command.time == 0:
                    ...
                    drone1_gnc.setCommandTrajectory(
                        75 * utils.knts2fps, 5 * utils.d2r, 15 * utils.d2r
                    )
                ...
                if drone1_gnc.time[-1] >= 2 * 60 and drone1_gnc.command.time == last_command_time[0]:
                    ...
                    drone1_gnc.setCommandTrajectory(
                        60 * utils.knts2fps, 0 * utils.d2r, -90 * utils.d2r
                    )
    ...
    fig, (ax_brg, ax_trk) = plt.subplots(2)
    ax_brg.plot(drone1_gnc.time, C2_drone1_tracker.target.range)
    ax_trk.plot(drone1_gnc.time, C2_drone1_tracker.target.bearing)
    figsavename = r'C:\Users\Alex\Dropbox\UA-MME\2023\Fall\ME-594\Project\figures\\'
    fig.savefig('track.png')

    # plt.show()

    # utils.plotSim(drone1_gnc, showPlots=True)
        # saveFolder=r"saved_simulations\figures\drone1",
        # filePrefix=f"stopTime")
    # if runSim:
    #     utils.save_obj(drone1_gnc, r"saved_simulations\drone1_6min.pkl")

    # utils.saveTrack(
    #     drone1_gnc, r"saved_simulations\tracks\drone1_6min.csv", downsample=50
    # )
