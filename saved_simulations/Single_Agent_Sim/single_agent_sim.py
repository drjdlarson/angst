...
def runsim(stopTime, saveSimulationFilePath=None, saveFiguresFolderPath=None):
    ...
    if runSim:
            ...
            while C2["time"][-1] < stopTime:
                ...
                for drone_name, drone_gnc in drones.items():
                    if not drone_gnc.crashed:
                        ...
                        if drone_gnc.time[-1] >= 120*cmdtime+drone_gnc.time[0] and first_command is False:
                            vel = 60*utils.knts2fps  # 60 knots
                            fpa = 1*utils.d2r  # 1 degree climb
                            hdg = 270*utils.d2r  # West-bound
                            drone_gnc.setCommandTrajectory(vel, fpa, hdg)
                            first_command = True
                            print('>-- First Command --<')
                        if drone_gnc.time[-1] >= 360*cmdtime+drone_gnc.time[0] and second_command is False:
        ...
        with utils.Timer(f"load_drone_objects"):
            drones = {}  # Reset drones
            drones[f'drone1'] = utils.load_obj(r"C:\Users\Alex\Documents\MAE 594 project\fangs\saved_simulations\Single_Agent_Sim\abs_command_drone1.pkl")
...
if __name__ == "__main__":
    with utils.Timer('OVERALL SIMULATION'):
        runsim(stopTime=25*60,
               saveSimulationFilePath=r'C:\Users\Alex\Documents\MAE 594 project\fangs\saved_simulations\Single_Agent_Sim',
               saveFiguresFolderPath=r'C:\Users\Alex\Documents\MAE 594 project\fangs\saved_simulations\Single_Agent_Sim')